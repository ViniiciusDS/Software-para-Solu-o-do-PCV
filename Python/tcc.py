...
def calcute_metodos_param(arquivo_matriz: str, incrementar_cidades: bool, num_cidades: int,
    ...
    DistMatriz = pd.DataFrame(np.zeros((50, 50)))
    ...
    for sim in range(n_simulacoes):
        ...
        DistMatriz_float = DistMatriz.iloc[:,:-1].astype(float)
        MatrizDistTrab = DistMatriz_float.values
    ...
    # Defina as cores das bordas das barras
    bordas = ['black', 'black', 'black']
    ...
    for i in range(3):
        ax2.bar(posicoes + i * largura_barra,
                Tab_Resultados_dist[:, i], width=largura_barra, label=metodos[i], color=cores[i], edgecolor=bordas[i])
